...
public class Parser {
	...
	/**
	 * This method executes a list of statements until an expected terminating
	 * string
	 * <p>
	 * This method is called as the main program loop with the parameters true
	 * and "" indicating EOF
	 * <p>
	 * 
	 * @param bExecuting
	 *            - Indicates if the method should execute the list of
	 *            statements
	 *
	 * @throws Exception
	 */
												...
												// EOF
		{
			...
			case Token.OPERAND:
				if (temp.subClassif != Token.IDENTIFIER)
					throw new ParserException(scanner.currentToken.iSourceLineNr,
							"Unexpected litteral value \'" + temp.tokenStr + "\'", scanner.sourceFileName);
				assignmentStmt(bExecuting);
				break;
			case Token.FUNCTION:
	 ...
	 * <p>
	 * for the example input: debug Expr on; On entering this method we expect
	 * to be on 'debug' On exiting this method we expect to be on ';' The result
	 * of this example should be to set the local value this.bShowExpr to true
	 * <p>
	 * 
	 * @param bExecuting
	 * @return
	...
	private ResultValue debugStmt(boolean bExecuting) throws ParserException {
		...
		default:
			throw new ParserException(scanner.currentToken.iSourceLineNr,
					"Non Implemented Debug type: \'" + scanner.currentToken.tokenStr + "\'", "scanner.sourceFileName");
		}
	 ...
	 * <p>
	 * On entering the method the currentToken should be on the function's
	 * identifier token. On exiting the method the currentToken should be on the
	 * token following ')' i.e one token after the end of the parameter list.
	 * 
	 * @param bExecuting
	 * @return
	 * @throws ParserException
	...
	private ResultValue evaluateFunction(boolean bExecuting) throws ParserException {
		...
		if (functionEntry instanceof STReference) // Dereference the item if its
													// a reference
	 ...
	 * <p>
	 * On entering the method the currentToken should be on '(' One leaving the
	 * method the currentToken should be on ')'
	 * 
	 * @param functionToken
	...
	private ResultValue functionCall(Token functionToken, STFunction functionSymbol) throws ParserException {
		...
		SymbolTable newTable = this.scanner.symbolTable.Clone(); // We need a
																	// copy of
																	// the
																	// symbol
																	// table
		StorageManager newStorageManager = new StorageManager(); // Storage
																	// manager
																	// is brand
																	// new
		...
		scanner.getNext();
		if (! scanner.currentToken.tokenStr.equals(")")) // check if we have arguments
			parameterList = argList(")");
		
		// Save off position
		returnPositionToken = scanner.currentToken;

		this.pushActivationRecord(record); // Push the activation record after
											// evaluating parameters

		if (parameterList.size() != functionSymbol.parmList.size())
			throw new ParserException(
					scanner.currentToken.iSourceLineNr, "Function \'" + functionToken.tokenStr + "\' has "
							+ functionSymbol.parmList.size() + " parameters, found " + parameterList.size(),
		...
		// Skip to function call, should now be on the ':' following function
		// definition
	...
	private ResultValue functionMaxElem(boolean bExecuting) throws ParserException {
		...
		{
			throw new ParserException(scanner.currentToken.iSourceLineNr, "Error missing \'(\' after LENGTH function",
					scanner.sourceFileName);
		}
		scanner.getNext();
	 ...
	 * <p>
	 * On entering the method the currentToken should be on 'Elem' On leaving
	 * the method the currentToken should be on ';'
	 * <p>
	 * 
	 * @param bExecuting
	 ...
	 * <p>
	 * On entering the method the currentToken should be on 'func' On exiting
	 * the method the currentToken should be on 'endfunc'
	 * <p>
	 * 
	 * @param bExecuting
	 * @throws ParserException
	...
	private void defineFunction(boolean bExecuting) throws ParserException {
		...
		scanner.getNext(); // Move past function name and onto the parameter
							// list
		...
		} else // CASE 2: parameters
		{
			...
			while (!scanner.currentToken.tokenStr.equals(")")) // Skip ',' and
																// exit if its a
																// ')'
			{
				...
				if (scanner.currentToken.tokenStr.equals("[")) // if it's an
																// array move
																// past the '['
																// and ']'
				{
			...
			this.environmentVector--; // reduce the environment vector again to
										// reset the state
		...
		functionSymbol = new STFunction(functionNameToken.tokenStr, returnTypeToken.tokenStr, Token.USER,
				this.environmentVector, paramList, scanner.currentToken, paramCount);
	 ...
	 * <p>
	 * On entering the method the currentToken should be on 'tuple' On exiting
	 * the method the currentToken should be on 'endtuple'
	 * <p>
	 * 
	 * @param bExecuting
	 * @throws ParserException
	...
	private void defineTuple(boolean bExecuting) throws ParserException {
		...
		scanner.getNext(); // Skip 'tuple' token, current token should now be on
							// the new tuple name
		...
		while (scanner.currentToken.subClassif != Token.END) {
			STEntry member;
			if (scanner.nextToken.tokenStr.equals("Ref")) // Could be a
															// reference type
															// TODO working here
			{
				...
				memberReference = new STReference(memberToken.tokenStr, memberToken.primClassif, memberToken.subClassif,
						this.environmentVector);
			...
			if (scanner.currentToken.tokenStr.equals(";")) // It was just a
															// plain variable
			{
				scanner.getNext(); // Skip the ';' and move on
			} else if (scanner.currentToken.tokenStr.equals("[")) // is an
																	// array,
																	// declare
																	// takes
																	// care of
																	...
																	// us
			{
				...
				if (scanner.currentToken.tokenStr.equals("unbounded")) {
					...
					declaredSize = expression("]"); // Read the expression
	 ...
	 * <p>
	 * On entering the method the currentToken should be on 'LENGTH' On leaving
	 * the method the currentToken should be on ';'
	 * <p>
	 * 
	 * @param bExecuting
	 ...
	 * @param bExecuting
	 * 
	 * @return Boolean: T - string is empty or only contains spaces F - string
	 * is non-empty
	 * 
	 * @throws ParserException
	 */
	private ResultValue functionSpaces(boolean bExecuting) throws ParserException {
		...
		if (!scanner.currentToken.tokenStr.equals("("))
			throw new ParserException(scanner.currentToken.iSourceLineNr, "Expected \'(\' after SPACES",
					scanner.sourceFileName);
	...
	private ResultValue functionMaxLength(boolean bExecuting) throws ParserException {
		...
		if (arg.type != Type.STRING) {
			throw new ParserException(scanner.currentToken.iSourceLineNr, "Arguement not of type String",
					scanner.sourceFileName);
		}

		scanner.getNext();
		if (!scanner.currentToken.tokenStr.equals(";"))
			throw new ParserException(scanner.currentToken.iSourceLineNr, "Expected \')\' after SPACES argument",
					scanner.sourceFileName);
	 ...
	 * <p>
	 * On entering the method the currentToken should be on 'print' On exiting
	 * the method the currentToken should be on ';'
	 * <p>
	...
	private void functionPrint(boolean bExecuting) throws ParserException {
		...
		if (!scanner.currentToken.tokenStr.equals("("))
			throw new ParserException(scanner.currentToken.iSourceLineNr, "Expected \'(\' after PRINT",
					scanner.sourceFileName);
	 ...
	 * <p>
	 * On entering this method the current token should be on 'else' On exiting
	 * this method the current token should be on 'endif'
	 * <p>
	 * 
	 * @param bExecuting
	 * @return
	...
	public ResultValue elseStmt(boolean bExecuting) throws ParserException {
		...
		if (scanner.currentToken.tokenStr.equals("endif") == false)
			throw new ParserException(scanner.currentToken.iSourceLineNr, "Expected \'endif\' after if statement",
					scanner.sourceFileName);
	...
	public ResultValue forStmt(boolean bExecuting) throws ParserException {
		...
		/*
		 * First, check what the next keyword is after starting/control value
		 * is. Since control value can only by one token, check the next token's
		 * string;
		 */
		switch (scanner.nextToken.tokenStr) {
		/*
		 * Counting for loop, First initialize control value, then evaluate the
		 * limit value(can be an expression), then evaluate the increment
		 * value(can be expression), finally use a for loop to iterate the
		 * correct number of times and process statements within the loop as we
		 * go.
		 */
		case "=":

			controlVal = evaluateOperand(scanner.currentToken); // Initialize
																// the control
																// value
			if (controlVal == null) // Value has not been declared so now we
									// need to declare it
			{
				intToken = new Token();
				intToken.tokenStr = Type.INT;
				controlVal = declareScalar(intToken, scanner.currentToken); // Implicitly
																			// declare
																			// the
																			// currentToken
																			// as
																			// an
																			// int
				STIdentifier newIdentifier = new STIdentifier(scanner.currentToken.tokenStr, Type.INT, Type.SCALAR,
						Type.VALUE, Type.LOCAL, this.environmentVector);

				scanner.symbolTable.putSymbol(scanner.currentToken.tokenStr, newIdentifier); // add
																								// symbol
																								// to
																								// table
			...
			// Since for loop assignment does not use semicolon, a slightly
			// modified version must be implemented here
			scanner.getNext(); // skip '='
			scanner.getNext();
			Utility.assign(this, controlVal, expression("to")); // Set control
																// value,
																// accounting
																// for
																// expressions
			...
			// Class voted changing incr or limit will NOT affect # of
			// iterations, so set them before evaluating statements
			// within the for loop.
			...
			for (int i = Integer.parseInt(controlVal.getInternalValue()); i < max; i += incr) {
				...
				controlVal.internalValue = String.valueOf(i); // Control value
																// is updated on
																// every
																// iteration
			...
			break;
		/*
		 * Iterating through strings using a delimiter for loop, similar to
		 * counting for loop: First initialize control value, then evaluate the
		 * string to iterate through(can be an expression), then evaluate the
		 * delimiter value(can be expression), finally use a for loop.
		 */
		case "from":
			/*
			 * As per specifications, the control value is implicitly declared
			 * if it has not been declared previously. However declareScalar
			 * only takes ';' as terminating character so this is a slight
			 * modification of it. First check if the control value exists, if
			 * it doesn't, create and put in storageManager; in either case,
			 * initialize it.
			 */
			if (scanner.symbolTable.getSymbol(scanner.currentToken.tokenStr) == null) {
				STIdentifier newIdentifier = new STIdentifier(scanner.currentToken.tokenStr, Type.STRING, Type.SCALAR,
						Type.VALUE, Type.LOCAL, this.environmentVector);
			...
			// Since class voted changing delimiter or string does NOT impact #
			// of iterations, first split the string,
			...
			break;
		/*
		 * Iterate over string or elements in array for loops. Again, similar to
		 * the other loops except, we need to check if the thing we're iterating
		 * over is an array or string to set the control value and for loop
		 * correctly.
		 */
		case "in":
			Token control = scanner.currentToken; // Hold the control value
													// token so it can be set
													// later
			...
			// Now set the control value, since we know the limitVal's type. If
			// control value doesn't exist, create it; otherwise initialize
			// control value
			if (scanner.symbolTable.getSymbol(control.tokenStr) == null) {
				STIdentifier newIdentifier = new STIdentifier(control.tokenStr, limitVal.type, Type.SCALAR, Type.VALUE,
						Type.LOCAL, this.environmentVector);

				scanner.symbolTable.putSymbol(control.tokenStr, newIdentifier); // add
																				// symbol
																				// to
																				// table
			...
			if (limitVal instanceof ResultList) {
				// As per the specs, changing # of array elements does not
				// affect # of iterations, so set it before looping
				max = ((ResultList) limitVal).iCurrentSize;
				for (int i = 0; i < max; i++) {
					scanner.jumpToPosition(forToken.iSourceLineNr, forToken.iColPos);
					/*
					 * As a personal choice, I decided to skip over holes in the
					 * array. This can be changed so it creates an error in the
					 * future...
					 */
			...
			// Class voted changing the string does not affect # of iterations,
			// so set it before looping
				...
				throw new ParserException(scanner.currentToken.iSourceLineNr,
						...
						scanner.sourceFileName);
			break;
		// Since the control value cannot be an expression itself, throw an
		// error if any unknown tokens appear within the for loop expression.
		default:
			throw new ParserException(scanner.currentToken.iSourceLineNr,
	 ...
	 * the method the currentToken should be on 'endwhile'
	 * <p>
	 * 
	 * @param bExecuting
	 * @return
	 ...
	 * <p>
	 * On entering the method the currentToken should be on 'if' On exiting the
	 * method the currenToken should be on ':'
	 * <p>
	 * 
	 * @param bExecuting
	 * @return - The
	 ...
	 * <p>
	 * On entering the method the currentToken should be on the first term in
	 * the expression On exiting the method the currentToken should be on the
	 * expected terminator
	 * <p>
	 * 
	 * @param expectedTerminator
	 *            - The token we are expecting to see at the end of the argument
	 *            list
	 * @return
	 * @throws ParserException
	 */
	 ...
	 * <p>
	 * Example: Int i; Assume currentToken is on "Int" when entering this
	 * method. This method should create a new entry into the symbol table and
	 * assign it the appropriate information.
	 * 
	 * <p>
	 * On exiting the method the current token should be on ';' OR '=' if an
	 * initialization statement follows
	 * 
	 * @throws ParserException
	 * 
	 */
	public void declareStmt(boolean bExecuting) throws ParserException {
		...
		storageManager.putVariableValue(variableToken.tokenStr, variableValue); // Insert
																				// the
																				// new
																				// empty
																				// variable
	...
	private ResultTuple declareTuple(Token typeToken, STTuple tupleDefinition) {
		...
		for (String identifierString : typeEntry.memberHash.keySet()) {
			...
			if (member instanceof STIdentifier) // Cast to identifier and insert
			{
				...
				if (identifierMember.structureType == Type.ARRAY) // Array
																	// inside of
																	// tuple
				{
					result.addMember(identifierString,
							new ResultList(new ResultValue(identifierMember.type), identifierMember.declaredSize));
				} else // Scalar
				{
			...
			// ---------------------------------------------------------------------
			// v v v
			else if (member instanceof STTuple) // Declare another tuple inside
												// of this one
			{
			...
			// ---------------------------------------------------------------------
			// v v v
			else if (member instanceof STReference) // Declare a reference as
													// part of this tuple
			{
				// Get the type, make tokens, pass them
				ResultReference ref = new ResultReference(((STReference) member).type);
				ref.referencedEntry = new STReference(member.symbol, member.primClassif, member.subClassif,
						this.environmentVector);
				ref.referencedEntry.referencedSymbol = scanner.symbolTable.getSymbol(((STReference) member).type);
	...
	/**
	 * Handles the creation of the relevant STIdentifier when building a
	...
	public STEntry declareIdentifier(boolean bExecuting) throws ParserException {
		...
		if (variableToken.tokenStr.equals("Ref")) {
			scanner.getNext(); // Move past Ref
			variableToken = scanner.nextToken;
			newReference = new STReference(variableToken.tokenStr, variableToken.primClassif, variableToken.subClassif,
					this.environmentVector);
		...
		if (scanner.nextToken.primClassif != Token.OPERAND && scanner.nextToken.subClassif != Token.IDENTIFIER) // If
																												// the
																												// next
																												// token
																												// is
		...
		{
			throw new ParserException(scanner.currentToken.iSourceLineNr,
					"Declaration of non identifier: " + scanner.nextToken.tokenStr, scanner.sourceFileName);
		}

		scanner.getNext(); // Move past the type declaration, should now be on
							// variable
		...
		if (Utility.isPrimitiveType(typeToken)) {
			...
			if (scanner.nextToken.tokenStr.equals("[")) {
				// Array stuff
				newIdentifier = new STIdentifier(variableToken.tokenStr, typeToken.tokenStr, Type.ARRAY, Type.VALUE,
						Type.LOCAL, this.environmentVector);
				newIdentifier.type = typeToken.tokenStr;
			} else {
				newIdentifier = new STIdentifier(variableToken.tokenStr, typeToken.tokenStr, Type.SCALAR, Type.VALUE,
						Type.LOCAL, this.environmentVector);
				newIdentifier.type = typeToken.tokenStr;
			}
	 ...
	 * <p>
	 * On entering the method the currentToken should be on the variable we want
	 * to declare On exiting the method the currentToken should be on ';' OR an
	 * assignment operator
	 * <p>
	 * 
	 * @param typeToken
	...
	private ResultList declareArray(Token typeToken, Token variableToken) throws ParserException {
		...
		if (scanner.nextToken.tokenStr.equals("unbounded")) // CASE 1 Unbounded
															// arrays may be
															// initialized or
															// maybe not
		{
			...
			// unbounded arrays need to skip the next few tokens and read the
			// intArgs
			...
			if (scanner.currentToken.tokenStr.equals("=")) // If the next
															// statement is an
															// assignment
			{
				scanner.getNext();
				...
				throw new ParserException(scanner.currentToken.iSourceLineNr,
						...
						scanner.sourceFileName);
		} else if (scanner.nextToken.tokenStr.equals("]")) // CASE 2 Array
															// without subscript
															// requires initial
															// values
		{
			...
			scanner.getNext();
			scanner.getNext(); // Move the currentToken onto the beginning of
								// the first expression
			if (scanner.currentToken.tokenStr.equals(";"))
				throw new ParserException(
						scanner.currentToken.iSourceLineNr, "Expected argument list for initilization of "
								+ typeToken.tokenStr + " array \'" + variableToken.tokenStr + "\'",
						scanner.sourceFileName);

			scanner.getNext();

			initArgs = argList(";"); // TODO working here
			
			if (initArgs.get(0) instanceof ResultList)
			{
				arrayValue = (ResultList) initArgs.get(0); // If its a result list just copy it over
				return arrayValue;
			}
			else
		...
		} else // CASE 3 There is an expression in between brackets
		{
															...
															// assignment
			{
				...
				{
					arrayValue = new ResultList(Utility.coerceToType(this, typeToken.tokenStr, initArgs.get(0)),
							arraySize);
				}
			}
		}
	 ...
	 * <p>
	 * When this function completes we will have parsed through until the end of
	 * a following expression. Current token will be on ';'
	 * <p>
	...
	public ResultValue assignmentStmt(boolean bExecuting) throws ParserException {
		...
		if (scanner.currentToken.subClassif != Token.IDENTIFIER) // Current
																	// token
																	// should be
																	// an
																	// identifier.
		...
		Token targetToken = scanner.currentToken; // Store the left hand side
													// token
		...
		if (targetResult == null) 
		{
			if (scanner.symbolTable.getSymbol(targetToken.tokenStr) != null)
				throw new ParserException(
						scanner.currentToken.iSourceLineNr, "Assigning to empty reference \'" + targetToken.tokenStr
								+ "\' " + "reference assignment requires \'Ref\' keyword before token",
						scanner.sourceFileName);
			else
				throw new ParserException(scanner.currentToken.iSourceLineNr,
						"Undeclared identifier: \'" + targetToken.tokenStr + "\'", scanner.sourceFileName);
		}
		...
		if (scanner.currentToken.primClassif != Token.OPERATOR) // Next token
																// should be an
																// operator.
		{
			throw new ParserException(scanner.currentToken.iSourceLineNr,
					"Expected assignment operator, found: " + scanner.currentToken.tokenStr, scanner.sourceFileName);
		}
		Token operatorToken = scanner.currentToken; // Store the operation we
													// are performing
		...
		if (!subResult1.terminatingStr.equals(";")) {
			throw new ParserException(scanner.currentToken.iSourceLineNr, "Missing \';\' after assignment statement",
					scanner.sourceFileName);
		}
		...
		} else if (!Utility.isNumeric(this, targetResult)) // If the target is
															// not numeric we
															// can't do any of
															// the following
															// assignments
		{
			throw new ParserException(scanner.currentToken.iSourceLineNr, "Can not perform operation \'"
					+ operatorToken.tokenStr + "\' on non numeric variable \'" + targetToken.tokenStr + "\'",
					scanner.sourceFileName);
		} else if (operatorToken.tokenStr.equals("-=")) {
	 ...
	 * <p>
	 * On entering the method the currentToken should be on 'Ref' On leaving the
	 * method the currentToken should be on ';'
	 * <p>
	 * The only expected form for this type of assignment is 'Ref' 'symbol' '='
	 * 'symbol'
	 * 
	 * @param bExecuting
	 * @return
	...
	private void referenceAssignment(boolean bExecuting) throws ParserException {
		...
		if (targetEntry instanceof STTuple) {
			...
			if (scanner.nextToken.tokenStr.equals(".")) // Check if we need to
														// grab a member
			{
				scanner.getNext();
				...
				if (targetEntry == null)
					throw new ParserException(
							scanner.currentToken.iSourceLineNr, resTuple.type + " tuple \'" + resTuple.symbol
									+ "\' has no member named \'" + scanner.currentToken.tokenStr + "\'",
							scanner.sourceFileName);

				ResultReference targetReferenceMember = (ResultReference) ((ResultTuple) this.callStack
						.get(resTuple.environmentVector).storageManager.getVariableValue(resTuple.symbol))
		...
		if (!scanner.currentToken.tokenStr.equals("=")) // expect to see an '='
														// here
			throw new ParserException(scanner.currentToken.iSourceLineNr,
		...
		if (valueEntry == null)
			throw new ParserException(scanner.currentToken.iSourceLineNr,
					"Symbol not found \'" + scanner.currentToken.tokenStr + "\'", scanner.sourceFileName);
		
		while(valueEntry instanceof STReference || valueEntry instanceof STTuple)
		{
			if (valueEntry instanceof STReference)
			{
				valueEntry = ((STReference) valueEntry).referencedSymbol;
			}
			if (scanner.nextToken.tokenStr.equals("."))
			{
				if (valueEntry instanceof STTuple)
				{
					scanner.getNext();
					scanner.getNext();
					valueEntry = ((STTuple)valueEntry).getMember(scanner.currentToken.tokenStr);
				}
			}
			else
				break;
		...
		targetReference.referencedSymbol = valueEntry; // Hook up the reference
														// // TODO problems with
														// function calls
														// overwriting the
														// reference?
	...
	private ResultValue stringIndexAssignment(ResultValue target) throws ParserException {
		...
		if (!index.type.equals(Type.INT)) {
			throw new ParserException(scanner.currentToken.iSourceLineNr,
					"String index must be of type int, found " + index.type, scanner.sourceFileName);
		}
		...
		newTarget.internalValue = Utility.insertString(target.internalValue, Integer.parseInt(index.internalValue),
				insert.internalValue);
	 ...
	 * <p>
	 * Expressions are in Infix notation so a stack will be used to parse. A
	 * stack of ResultValue will be used for the output
	 * <p>
	 * Assume currentToken is on the first operand of the expression on entering
	 * the method. When this method completes, the current token will be on the
	 * separator indicating the end of the expression.
	 * <p>
	 * 
	 * @return
	 * @throws Exception
	...
	public ResultValue expression(String expectedTerminator) throws ParserException {
																												...
																												// terminator
		{
			...
			case Token.OPERATOR:
				bOperatorFound = true;
				if (expected == Token.OPERAND && scanner.currentToken.tokenStr.equals("-")) // If
																							// we
																							// are
																							// looking
																							// for
																							...
																							// u-
				{
					...
					temp = scanner.currentToken;
					sToken = new StackToken(temp);
				} else if (expected != Token.OPERATOR) // Error Unexpected
														// operator
					throw new ParserException(scanner.currentToken.iSourceLineNr,
				...
				if (operatorStack.isEmpty()) {
					...
					if (sToken.iPrecedence > operatorStack.peek().iStackPrecedence) // If
																					// precedence
																					// is
																					// greater
																					// push
					...
					} else // Otherwise loop until it is not the case
					{
						while (!operatorStack.isEmpty()
								&& sToken.iPrecedence <= operatorStack.peek().iStackPrecedence) {
							popped = operatorStack.pop();
							if (popped.iOperandCnt == 1) // Unary operator is a
															// unique case
							{
								tempRes01 = outputStack.pop(); // get a single
																// operand
								tempRes02 = Utility.evaluateUnaryOperator(this, tempRes01, popped.token);
								outputStack.push(tempRes02); // Push result back
																// onto stack
							} else {
								tempRes02 = outputStack.pop(); // Get two
																// operands for
																// our operator
								...
								outputStack.push(tempRes03); // Push result back
																// onto stack
							}
						}

						operatorStack.push(sToken); // Then push the new
													// operator on
					}
				}
			...
			case Token.SEPARATOR:
				...
				case "(":
					...
					scanner.getNext();
					tempRes01 = expression(")"); // evaluate the expression up
													// to the rparen
		...
		while (!operatorStack.isEmpty()) {
			...
			if (popped.iOperandCnt == 1) // Unary operator is a unique case
			{
				...
				tempRes02 = outputStack.pop(); // Get two operands for our
												// operator
				tempRes01 = outputStack.pop();
		...
		if (outputStack.isEmpty())
			throw new ParserException(scanner.currentToken.iSourceLineNr, "Expression parse missmatch",
					scanner.sourceFileName);
	...
	private ResultValue evaluateOperand(Token operandToken) throws ParserException {
		...
		if (operandToken.primClassif != Token.OPERAND) {
			throw new ParserException(scanner.currentToken.iSourceLineNr,
					"Token \'" + operandToken.tokenStr + "\' is not an operand", scanner.sourceFileName);
		}
		...
		case Token.IDENTIFIER:
			...
			if (resultEntry instanceof STReference) // Dereference the item if
													// its a reference
			...
			while (res instanceof ResultTuple || res instanceof ResultList) {
				if (res instanceof ResultTuple) {
					...
					if (scanner.nextToken.tokenStr.equals(".")) // Check if we
																// need to grab
																// a member
					{
						scanner.getNext();
						...
						if (res == null)
							throw new ParserException(scanner.currentToken.iSourceLineNr,
									resTuple.type + " tuple \'" + operandToken.tokenStr + "\' has no member named \'"
											+ scanner.currentToken.tokenStr + "\'",
									scanner.sourceFileName);

						if (res instanceof ResultReference) {
							ResultValue tempRes = this.callStack
									.get(((ResultReference) res).referencedEntry.environmentVector).storageManager
											.getVariableValue(
													((ResultReference) res).referencedEntry.referencedSymbol.symbol);
							if (tempRes == null)
								return res.Clone();
							else
								res = tempRes;
						...
						break;
				} else if (res instanceof ResultList) // If the element is an
														// array
				{
					if (scanner.nextToken.tokenStr.equals("[")) // Check if we
																// need to grab
		...
		default:
			throw new ParserException(scanner.currentToken.iSourceLineNr,
					"Unknown identifier type \'" + operandToken.tokenStr + "\' found", scanner.sourceFileName);
		}
		return res;
	}
}
