...
public class ResultList extends ResultValue implements Cloneable {
	...
	public void insert(Parser parser, int index, ResultValue value) throws ParserException {
		...
		if (index >= this.iMaxSize || index < 0)
			throw new ParserException(parser.scanner.currentToken.iSourceLineNr, "Array index out of bounds: " + index,
					parser.scanner.sourceFileName);

		if (this.iMaxSize == Type.ARRAY_UNBOUNDED && this.internalValueList.length < index) // Unbounded
																							// array
																							// needs
																							// to
																							// grow
	...
	public void set(Parser parser, ResultValue newValue) throws ParserException {
		...
		if (newValue instanceof ResultList) {
			...
			for (int i = 0; i < size; i++) // Replace all default values while
											// leaving others
			{
